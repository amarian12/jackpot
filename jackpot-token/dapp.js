...
var Token = web3.eth.contract(tokenABI).at(tokenReward);
...
function contributeETH(ETH) {
  return new Promise(function(resolve, reject) {
    ...
    Token.transfer(contractAddress,new web3.BigNumber(ETH).mul(1e18), function(error, data) {
      if (error) {
        console.log(error.message);
        return reject(error);
      }
...
getPlayers().then(function(players) {
  ...
  $("#eth-balance").html(" : " + JackpotBalance);
})
}
render();
...
addresses.forEach(function(address, i) {
  ...
  $.alert({
    useBootstrap:false,
    content:"<h1>Someone Won The Jackpot!</h1>",
    animateFromElement:false,
    title:"    ",
    boxWidth:"530px",
    onOpenBefore: function () {
