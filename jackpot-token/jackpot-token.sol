...
interface token { 
  ...
  function balanceOf(address) constant external returns (uint256);
}
...
contract Jackpot is Ownable {
  ...
  function executeLottery() public { 
        ...
        if (lastIndex >= numberOfPlayers) {
          uint randomNumber = tokenReward.balanceOf(address(this)).mul(16807) % 2147483647;
        ...
        //   if (!owner.send(balance/10)) throw;
          tokenReward.transfer(owner,balance/10);
          //Both SafeMath.div and / throws on error
        //   if (!winner.send(balance - balance/10)) throw;
        tokenReward.transfer(winner,balance.sub(balance/10));
  ...
  function tokenFallback (address _from, uint256 _value) public {
    require(msg.sender == addressOfTokenUsedAsReward);
    ...
    ticketsMap[_from].push(senderTickets);

    contributions[_from] = contributions[_from].add(weiAmount);

    emit newContribution(_from, weiAmount);
